...
required.add_argument("--program", dest="program", help="program from which vcf came from", required=True, metavar="PROGRAM") #Options == GATK, Samtools, FreeBayes
...
#				AAFlag1 = 1
#				continue
		if qualSubset[0] == "AC" or qualSubset[0] == "AC1":
			AC = qualSubset[1]	
		if qualSubset[0] == "AF" or qualSubset[0] == "AF1":
...
#		continue
	...
	if AF is None:
		...
		continue
	if str(args.program) == "GATK" or str(args.program) == "FreeBayes":
		if AN is None:
...
#	sys.stderr.write(DP + " " + AN + " \n")
	...
	elif str(args.program) == "Samtools":
		Depth.extend([float(DP)])
	else:
		sys.exit("Error3c -- program is neither GATK, Samtools or FreeBayes (" + str(args.program) +"). Exiting...\n")	
...
#	HaploScore.extend([HaplotypeScore])
	...
	if (AF <= .01):
		...
		sys.exit("Error3b -- AF is nowhere between 0 and 1 (" + str(AF) + "). Exiting...\n")
...
print "AF 0_.01: " + str(AF_0_01) + " AF .01_.05: " + str(AF_01_05) + " AF .05_.1: " + str(AF_05_10) + " AF .10_.25: " + str(AF_10_25) + " AF .25_.50: " + str(AF_25_50) + " AF .5_1: " + str(AF_50_100)
