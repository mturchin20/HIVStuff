...
Data8 <- read.table(paste(cmd_args[5], ".All.", cmd_args[6], ".maf1.skat0.perm1.epacts", sep=""), header=TRUE)
...
jpeg(file=cmd_args[8], height=2750, width=2750, res=300)
#pdf(file=cmd_args[8], height=7, width=7)
par(mar=c(5, 5, 4, 2) + 0.1)

plot(-1*log10(xVals1), -1*log10(Data1subset[,10]), ylim=c(0,3.75), xlab="-log10 Expected p-Values", ylab="-log10 Observed p-Values", main=cmd_args[7], type="b", col="RED", cex.main=2.3, cex.lab=2.3, cex.axis=2.3, cex=2)

abline(a=0, b=1, lwd=2)
lines(-1*log10(c(1:nrow(Data2subset))/(nrow(Data2subset)+1)), -1*log10(Data2subset[,10]), type="b", col="BLUE", cex=2)
lines(-1*log10(c(1:nrow(Data3subset))/(nrow(Data3subset)+1)), -1*log10(Data3subset[,10]), type="b", col="GREEN", cex=2)
lines(-1*log10(c(1:nrow(Data4subset))/(nrow(Data4subset)+1)), -1*log10(Data4subset[,10]), type="b", col="PURPLE", cex=2)
lines(-1*log10(c(1:nrow(Data5subset))/(nrow(Data5subset)+1)), -1*log10(Data5subset[,10]), type="b", col="YELLOW", cex=2)
lines(-1*log10(c(1:nrow(Data6subset))/(nrow(Data6subset)+1)), -1*log10(Data6subset[,10]), type="b", col="ORANGE", cex=2)
lines(-1*log10(c(1:nrow(Data7subset))/(nrow(Data7subset)+1)), -1*log10(Data7subset[,10]), type="b", col="BROWN", cex=2)
lines(-1*log10(c(1:nrow(Data8subset))/(nrow(Data8subset)+1)), -1*log10(Data8subset[,10]), type="b", col="BLACK", cex=2)

legend("bottomright", c("All", "Exonic", "Nonsynonymous", "PP2_PD", "PP2_D", "Sift_D", "LRT_D", "All_Perm"), pch=c(1,1,1,1,1,1,1,1), col=c("RED", "BLUE", "GREEN", "PURPLE", "YELLOW", "ORANGE", "BROWN", "BLACK"), cex=1.5)
