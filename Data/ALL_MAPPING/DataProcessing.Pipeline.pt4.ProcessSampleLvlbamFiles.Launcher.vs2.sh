...
#		baseFile1=`echo $fileDirList | perl -F/\;/ -ane 'my @vals1 = split(/\|/, $F[0]); if ($vals1[1] =~ m/(.*)\.sff/) { print $1; };'`
		baseFile1=`echo $fileDirList | perl -F/\;/ -ane 'my @vals1 = split(/\|/, $F[0]); my @vals2 = split(/\//, $vals1[0]); if ($vals2[10] =~ m/(POOL\w+).*(\d\dRL)/) { print $1, "_", $2; } '`
...
#		echo $baseFile1
		...
		DirLvl1=`echo $mainDir1 | perl -F/ -lane '$F[8] = "PostMerge"; print "/", join("/", @F[1..9]);'`
		DirLvl2=`echo $mainDir1 | perl -F/ -lane '$F[8] = "PostMerge"; if ($F[10] =~ m/(POOL\w+).*(\d\dRL)/) { $F[10] = $1 . "_" . $2;  print "/", join("/", @F[1..10]);}'`
		mainDir2=$DirLvl2
		...
		qsubout="${mainDir2}/qsubout/qsubout.${baseFile1}.txt"
		qsubouttemp="${mainDir2}/qsubout/qsubout.${baseFile1}.txt_temp"
		qsuboutAll="${mainDir2}/qsubout/qsubout.All.${baseFile1}.txt"
		qsuboutAlltemp="${mainDir2}/qsubout/qsubout.All.${baseFile1}.txt_temp"
...
#		qsub -j oe -o $qsubout -e $qsubout -l walltime=72:00:00,mem=6gb,nodes=1:ppn=32 -v mainDir1=$mainDir2,baseFileN=$baseFile1,refFileFlag1=$refFileFlag1 DataProcessing.Pipeline.pt4.ProcessSampleLvlbamFiles.vs1.sh 
		qsub -j oe -o $qsubout -e $qsubout -l walltime=72:00:00,mem=6gb,nodes=1:ppn=6 -v mainDir1=$mainDir2,baseFileN=$baseFile1,refFileFlag1=$refFileFlag1 DataProcessing.Pipeline.pt4.ProcessSampleLvlbamFiles.vs2.sh 
