...
coverageBedFilePreQCFull=`samtools depth -b /home/michaelt/LabStuff/StephensLab/HIVStuff/PG/120326_HG19_SW_MM5_EZ_HX3/120326_HG19_SW_MM5_EZ.altrd.bed $baseFileN.QCed.preGATK.QCed.samplesMerged.bam | perl -lane '$sum += $F[2]; $count1++; if (eof()) { print $sum, "\t", $count1, "\t", $sum/$count1; } '`
...
coverageBedFilePostQCFull=`samtools depth -b /home/michaelt/LabStuff/StephensLab/HIVStuff/PG/120326_HG19_SW_MM5_EZ_HX3/120326_HG19_SW_MM5_EZ.altrd.bed $baseFileN.QCed.preGATK.QCed.samplesMerged.rmdup.BQSR.calmd.bam | perl -lane '$sum += $F[2]; $count1++; if (eof()) { print $sum, "\t", $count1, "\t", $sum/$count1; } '`
