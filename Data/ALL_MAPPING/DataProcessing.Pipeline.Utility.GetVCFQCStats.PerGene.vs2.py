...
for geneID in hash1:
	...
	sys.stderr.write(str(geneID) + "," + str(hash1[geneID]["NumVar"]) + "," + str(hash1[geneID]["Exonic"]) + "\n")

	if hash1[geneID]["NumVar"] == 0:
		sys.stderr.write("Error 3a -- geneID " + str(geneID) + " has 0 variants (" + str(hash1[geneID]["NumVar"]) + "). Skipping...\n")
		continue
	if hash1[geneID]["Exonic"] == 0:
		sys.stderr.write("Error 3a -- geneID " + str(geneID) + " has 0 Exonic variants (" + str(hash1[geneID]["Exonic"]) + "). Adding 1...\n")
		hash1[geneID]["Exonic"] = 1
