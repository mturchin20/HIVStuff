...
for line2 in file2:
	...
	if ChrBP2 in hash1:
		for i in range(0,len(hash1[ChrBP2])):
		
			line2Temp = copy.copy(line2)

#			sys.stderr.write("here!\n")

			SNVClass = hash1[ChrBP2][i][27].split()
			SNVClassUse = "_".join(SNVClass)

			line2Temp[4] = hash1[ChrBP2][i][len(hash1[ChrBP2][i])-2]
			line2Temp[5] = hash1[ChrBP2][i][len(hash1[ChrBP2][i])-1]

			line2Temp.extend([hash1[ChrBP2][i][25], hash1[ChrBP2][i][26], SNVClassUse, hash1[ChrBP2][i][28], hash1[ChrBP2][i][34], hash1[ChrBP2][i][36], hash1[ChrBP2][i][38], hash1[ChrBP2][i][40]])
