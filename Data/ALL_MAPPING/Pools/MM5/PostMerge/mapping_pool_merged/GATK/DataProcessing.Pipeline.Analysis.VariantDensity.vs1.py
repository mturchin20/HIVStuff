...
#	randVal1 = random.randint(0,len(hash1[GeneEntries])-1)
	randVal1 = random.randint(0,len(transcriptsWTopExonCount)-1)
#	sys.stderr.write(str(randVal1) + "\n")
#	hash1Temp[GeneEntries] = hash1[GeneEntries][random.randint(0,len(hash1[GeneEntries])-1)]
#	hash1Temp[GeneEntries] = hash1[GeneEntries][randVal1]
	hash1Temp[GeneEntries] = transcriptsWTopExonCount[randVal1]

#	if len(hash1[GeneEntries]) > 1 and count1 < 2:
#		sys.stderr.write(str(randVal1) + "\t" + str(hash1[GeneEntries][randVal1]) + "\n")
#		count1 += 1
...
for line2 in file2:
	...
	line2[34] = int(line2[34])

	if line2[18] in hash2:
		hash2[line2[18]].append(line2)
	else:
		hash2[line2[18]] = [line2]
...
for GeneID1 in hash2:
	if len(hash2[GeneID1]) > 1:
		if GeneID1 in hash1:
			Differences = [GeneID1]
#			sys.stderr.write("Test1a:\t" + str(hash2[GeneID1][0][34]) + "\t" + str(hash2[GeneID1][1][34])  + "\n")
			hash2[GeneID1].sort(key=operator.itemgetter(34))
#			sys.stderr.write("Test1b:\t" + str(hash2[GeneID1][0][34]) + "\t" + str(hash2[GeneID1][1][34])  + "\n")
...
#					#Three situations per position -- the variant is less than the end of the previous exon, so you're done. The variant is in between the previous exon and current exon, so extend the end of the previous exon to include the variant (which will affect all downstream variants but should not affect upstream variants already analyzed). And the variant is greater than the start of the current exon, so subtract the difference between the beginning of the current exon and end of the last exon to remove the length of the intron. This process begins at loop index 1, not 0, because the first 'previous exon' to be analyzed should be the first one. And this process ends with the index on the last exon because if the variant is greater than the beginning of the last exon, you correct for the intron one last time and then there will be nothing else to do, so the LessThanFlag1 can still be 0 and you will just end the loop
					...
					if LessThanFlag1 == 0:
						...
						if (hash2[GeneID1][i][34] < hash1[GeneID1][9][j-1]): 
							LessThanFlag1 = 1
#							sys.stderr.write("Happening1\t" + GeneID1 + "\t" + str(hash2[GeneID1][i][34]) + "\t" + str(hash1[GeneID1][9][j-1]) + "\n")
						...
						elif ((hash2[GeneID1][i][34] > hash1[GeneID1][9][j-1]) and (hash2[GeneID1][i][34] < hash1[GeneID1][8][j])):
#							sys.stderr.write(GeneID1 + "\tHappening2a\t" + str(hash1[GeneID1][9][j-1]) + "\t" + str(hash2[GeneID1][i][34]) + "\t" + str(hash1[GeneID1][8][j]) + "\n")
							hash1[GeneID1][9][j-1] = hash2[GeneID1][i][34]
#							sys.stderr.write(GeneID1 + "\tHappening2b\t" + str(hash1[GeneID1][9][j-1]) + "\t" + str(hash2[GeneID1][i][34]) + "\t" + str(hash1[GeneID1][8][j]) + "\n")
						...
						elif (hash2[GeneID1][i][34] > hash1[GeneID1][8][j]):
#							sys.stderr.write(GeneID1 + "\tHappening3a\t" + str(i) + "," + str(j) + "\t" + str(hash2[GeneID1][i][34]) + "\t" + str(hash1[GeneID1][8][j]) + "\t" + str(hash1[GeneID1][9][j-1]) + "\t" + str(hash1[GeneID1][8][j] - hash1[GeneID1][9][j-1]) + "\n")
							hash2[GeneID1][i][34] -= int(hash1[GeneID1][8][j]) - int(hash1[GeneID1][9][j-1])
#							sys.stderr.write(GeneID1 + "\tHappening3b\t" + str(i) + "," + str(j) + "\t" + str(hash2[GeneID1][i][34]) + "\t" + str(hash1[GeneID1][8][j]) + "\t" + str(hash1[GeneID1][9][j-1]) + "\t" + str(hash1[GeneID1][8][j] - hash1[GeneID1][9][j-1]) + "\n")
						else:
							Val1=0
				if (i > 0):
					Differences.append(str(hash2[GeneID1][i][34] - hash2[GeneID1][i-1][34]))
