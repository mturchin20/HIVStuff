...
#			sys.stderr.write("Test1b:\t" + str(hash2[GeneID1][0][42]) + "\t" + str(hash2[GeneID1][1][42])  + "\n")
			for i in range(0, len(hash2[GeneID1])): #Gong down rows of variants for the current gene
				LessThanFlag1 = 0
				for j in range(1, len(hash1[GeneID1][8])-1): #Going across start/ends of exons to correct basepair position of variant if need be
...
#							sys.stderr.write(GeneID1 + "\tHappening2a\t" + str(hash1[GeneID1][9][j-1]) + "\t" + str(hash2[GeneID1][i][42]) + "\t" + str(hash1[GeneID1][8][j]) + "\n")
							EndDiff = hash2[GeneID1][i][42] - int(hash1[GeneID1][9][j-1])
							StartDiff = int(hash1[GeneID1][8][j]) - hash2[GeneID1][i][42]
							if (EndDiff > StartDiff):
								hash1[GeneID1][8][j] = hash2[GeneID1][i][42]
								adjustments.append(str(StartDiff) + "_2")
							elif (StartDiff >= EndDiff):
								hash1[GeneID1][9][j-1] = hash2[GeneID1][i][42]		
								adjustments.append(str(EndDiff) + "_1")
							else:
								sys.stderr.write("Error1a -- EndDiff is neither > nor <= StartDiff (" + str(StartDiff) + ", " + str(EndDiff) + "\n")
...
#							sys.stderr.write(GeneID1 + "\tHappening3b\t" + str(i) + "," + str(j) + "\t" + str(hash2[GeneID1][i][42]) + "\t" + str(hash1[GeneID1][8][j]) + "\t" + str(hash1[GeneID1][9][j-1]) + "\t" + str(hash1[GeneID1][8][j] - hash1[GeneID1][9][j-1]) + "\n")
			...
			for k in range(0,len(hash1[GeneID1][8])-1):
				try:
					fullLength += float(hash1[GeneID1][9][k]) - float(hash1[GeneID1][8][k])
				except ValueError:
					sys.stderr.write(GeneID1 + "\t" + ",".join(map(str, hash1[GeneID1][8])) + "\t" + ",".join(map(str, hash1[GeneID1][9])) + "\n")
			Differences.insert(1, fullLength)
			Differences.insert(2, ",".join(adjustments))
			totalDifferences.append(Differences)

for k in range(0, len(totalDifferences)):
	print totalDifferences[k][0] + "\t" + str(totalDifferences[k][1]) + "\t" + str(totalDifferences[k][2]) + "\t" + ",".join(totalDifferences[k][3:len(totalDifferences[k])])
