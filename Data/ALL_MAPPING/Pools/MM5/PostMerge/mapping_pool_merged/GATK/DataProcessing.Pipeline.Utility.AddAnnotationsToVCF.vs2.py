	...
	if re.search('^#', line2[0]):
		...
		if ChrBP2 in hash1:
			for SNPinfo in hash1[ChrBP2]:
				SNPinfoVCF = SNPinfo[8].split(";")

				VCFac = 0

				for VCFentry in SNPinfoVCF:
					if re.search('^AC=', VCFentry):
						ACvals = VCFentry.split('=')
						VCFac = ACvals[1]

#				sys.stderr.write(str(VCFac) + "\n")

				if len(SNPinfo[27].split()) > 1:
					SNPinfo[27] = "_".join(SNPinfo[27].split())

				SNPinfoAdd = ";".join(SNPinfo[25:29])
		
				if args.AC:
#					sys.stderr.write(str(SNPinfo[17]) + "\n")
#					if int(SNPinfo[17]) <= int(args.AC) and int(SNPinfo[17]) > 0:	
					if int(VCFac) <= int(args.AC) and int(VCFac) > 0:	
...
#						if genoPresent == 1:
#						if int(SNPinfo[17]) == numCopies:
						if int(VCFac) == numCopies:
							sys.stderr.write(str(SNPinfo[17]) + "\t" + str(VCFac) + "\t" + str(numCopies) + "\n")
