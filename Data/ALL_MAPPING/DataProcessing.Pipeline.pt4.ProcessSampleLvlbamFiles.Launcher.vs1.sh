...
#		echo $baseFile1
		...
		DirLvl1=`echo $mainDir1 | perl -F/ -lane '$F[7] = "PostMerge"; print "/", join("/", @F[1..8]);'`
		DirLvl2=`echo $mainDir1 | perl -F/ -lane '$F[7] = "PostMerge"; if ($F[9] =~ m/(POOL\w+).*(\d\dRL)/) { $F[9] = $1 . "_" . $2;  print "/", join("/", @F[1..9]);}'`
		mainDir2=$DirLvl2
