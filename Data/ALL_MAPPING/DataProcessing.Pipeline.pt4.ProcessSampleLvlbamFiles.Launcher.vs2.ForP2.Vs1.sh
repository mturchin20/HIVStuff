...
do
	...
	if [ $FLAG1 == 1 ] ; then
		...
		DirLvl1=`echo $mainDir1 | perl -F/ -ane '$F[9] = "PostMerge"; print "/", join("/", @F[1..9]);'`
		DirLvl2=`echo $mainDir1 | perl -F/ -ane '$F[9] = "PostMerge"; if ($F[10] =~ m/(POOL_P2_[0-9]+).*(\d\dRL)/) { $F[10] = $1 . "_" . $2;  print "/", join("/", @F[1..10]);}'`
		mainDir2=$DirLvl2
...
#		qsub -j oe -o $qsubout -e $qsubout -l walltime=72:00:00,mem=6gb,nodes=1:ppn=32 -v mainDir1=$mainDir2,baseFileN=$baseFile1,refFileFlag1=$refFileFlag1 DataProcessing.Pipeline.pt4.ProcessSampleLvlbamFiles.vs1.sh 
		qsub -j oe -o $qsubout -e $qsubout -l walltime=72:00:00,mem=6gb,nodes=1:ppn=6 -v mainDir1=$mainDir2,baseFileN=$baseFile1,refFileFlag1=$refFileFlag1 DataProcessing.Pipeline.pt4.ProcessSampleLvlbamFiles.vs2.ForP2.vs1.sh 
