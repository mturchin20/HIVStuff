...
POOLID=`echo $mainDir1 | perl -F/ -ane 'if ($F[9] =~ m/(POOL[a-zA-Z0-9]+).*(\d\dRL)/) { print $1, ":", $2; }'` 
...
#/home/shared/software/bwa/bwa-0.7.4/bwa mem -M -R "@RG\tID:${baseFile1}\tSM:${POOLID}\tLB:${libraryID}\tPL:LS454" $refFile2 $baseFile1.QCed.fastq.gz | gzip > $baseFile1.QCed.sam.gz #Including RG information header stuff
