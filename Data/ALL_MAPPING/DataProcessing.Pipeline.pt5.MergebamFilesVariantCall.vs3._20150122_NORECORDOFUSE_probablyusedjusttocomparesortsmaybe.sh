...
#	mergeBamCommand1=`cat $mergeFileList1 | perl -F/\;/ -lane 'my @vals1 = split(/\|/, $F[0]); $vals1[7] = "PostMerge"; if ($vals1[9] =~ m/(POOL\d+).*(\d\dRL)/) { $F[vals1] = $1 . "_" . $2;  print "I=/", join("/", @vals1[1..9]), "/", $vals1[9], ".QCed.preGATK.QCed.samplesMerged.rmdup.BQSR.calmd.bam\t";}'`
	mergeBamCommand1=`cat $mergeFileList1 | perl -F/\;/ -lane 'my @vals1 = split(/\|/, $F[0]); my @vals2 = split(/\//, $vals1[0]); $vals2[8] = "PostMerge"; if ($vals2[10] =~ m/(POOL\w+).*(\d\dRL)/) { $vals2[$#vals2] = $1 . "_" . $2;  print "/", join("/", @vals2[1..10]), "/", $vals2[10], ".QCed.preGATK.QCed.samplesMerged.rmdup.BQSR.calmd.bam\t";}'`

	echo $mergeBamCommand1

#	/home/shared/software/java/jre1.7.0_09/bin/java -Xmx50g -jar /home/shared/software/picard/picard-tools-1.91/MergeSamFiles.jar $mergeBamCommand1 O=${mainDir1}/AllPools.Vs2.QCed.preGATK.QCed.samplesMerged.rmdup.BQSR.calmd.AllPoolsMerged.bam USE_THREADING=TRUE SORT_ORDER=coordinate
	/home/shared/software/samtools/samtools-0.1.19/samtools merge -r ${mainDir1}/AllPools.Vs2.QCed.preGATK.QCed.samplesMerged.rmdup.BQSR.calmd.AllPoolsMerged.bam.SamtoolsMerge $mergeBamCommand1

	/home/shared/software/samtools/samtools-0.1.19/samtools index ${mainDir1}/AllPools.Vs2.QCed.preGATK.QCed.samplesMerged.rmdup.BQSR.calmd.AllPoolsMerged.bam.SamtoolsMerge ${mainDir1}/AllPools.Vs2.QCed.preGATK.QCed.samplesMerged.rmdup.BQSR.calmd.AllPoolsMerged.SamtoolsMerge.bai
