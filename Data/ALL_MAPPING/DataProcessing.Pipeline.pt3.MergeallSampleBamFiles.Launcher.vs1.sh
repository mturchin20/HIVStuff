...
#	echo $baseFile1
	...
	DirLvl1=`echo $mainDir1 | perl -F/ -ane '$F[7] = "PostMerge"; print "/", join("/", @F[1..8]);'`
	DirLvl2=`echo $mainDir1 | perl -F/ -ane '$F[7] = "PostMerge"; if ($F[9] =~ m/(POOL\w+).*(\d\dRL)/) { $F[9] = $1 . "_" . $2;  print "/", join("/", @F[1..9]);}'`
...
#	echo $fileDirList
	...
	qsub -j oe -o $qsubout -e $qsubout -l walltime=72:00:00,mem=6gb,nodes=1:ppn=6 -v fileDirList1=$fileDirList DataProcessing.Pipeline.pt3.MergeallSampleBamFiles.vs3.sh 
