...
if [ $refFileFlag1  == 0 ] ; then
	...
	mergeBamCommand1=`cat $mergeFileList1 | perl -F/\;/ -lane 'my @vals1 = split(/\|/, $F[0]); my @vals2 = split(/\//, $vals1[0]); $vals2[9] = "PostMerge"; if ($vals2[10] =~ m/(POOL_P2_[0-9]+).*(\d\dRL)/) { $vals2[$#vals2] = $1 . "_" . $2;  print "I=/", join("/", @vals2[1..10]), "/", $vals2[10], ".QCed.preGATK.QCed.samplesMerged.rmdup.BQSR.calmd.bam\t";}'`

	echo $mergeBamCommand1

	/home/shared/software/java/jre1.7.0_09/bin/java -Xmx50g -jar /home/shared/software/picard/picard-tools-1.91/MergeSamFiles.jar $mergeBamCommand1 O=${mainDir1}/AllPools.P2.Vs1.AllPoolsMerged.bam USE_THREADING=TRUE SORT_ORDER=coordinate

	/home/shared/software/samtools/samtools-0.1.19/samtools index ${mainDir1}/AllPools.P2.Vs1.AllPoolsMerged.bam ${mainDir1}/AllPools.P2.Vs1.AllPoolsMerged.bai
