...
POOLID=`echo $mainDir1 | perl -F/ -ane 'if ($F[10] =~ m/(POOL_P2_[0-9]+).*(\d\dRL)/) { print $1, "_", $2; }'` 
libraryID=`echo $mainDir1 | perl -F/ -ane 'if ($F[10] =~ m/(POOL_P2_[0-9]+).*(\d\dRL)/) { print $1; }'` 

/data/userdata/pg/michaelt/Data/ALL_MAPPING/Pools/P2/PreMerge/POOL_P2_100-01RL
...
coverageBedFilePostQCFull=`samtools depth -b /home/michaelt/LabStuff/StephensLab/HIVStuff/PG/OID41305_HG19_Match5_probes/OID41305_HG19_Match5_probe_coverage.altrd.bed $baseFile1.QCed.preGATK.QCed.bam | perl -lane '$sum += $F[2]; $count1++; if (eof()) { print $sum, "\t", $count1, "\t", $sum/$count1; } '`
