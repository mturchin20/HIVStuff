...
mainDir2=`echo $fileDirList1 | perl -F/\;/ -ane 'chomp(@F); my @vals1 = split(/\|/, $F[0]); print $vals1[0];' | perl -F/ -ane '$F[8] = "PostMerge"; if ($F[10] =~ m/(POOL\w+).*(\d\dRL)/) { $F[10] = $1 . "_" . $2;  print "/", join("/", @F[1..10]);}'`
baseFileN=`echo $mainDir2 | perl -F/ -ane 'print $F[10];'`
POOLID=`echo $mainDir2 | perl -F/ -ane 'if ($F[10] =~ m/(POOL[a-zA-Z0-9]+).*(\d\dRL)/) { print $1, "_", $2; }'` 
